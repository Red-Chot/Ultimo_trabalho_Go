...
func (ps *PlayerService) AddPlayer(nickname string, life, attack, armor int) (*entity.Player, error) {
	...
	if armor > 10 || armor <= 0 {
		return nil, errors.New("player armor must be between 1 and 10")
	}
	...
	player = entity.NewPlayer(nickname, life, attack, armor)
...
func (ps *PlayerService) SavePlayer(id, nickname string, life, attack, armor int) (*entity.Player, error) {
	player, err := ps.PlayerRepository.LoadPlayerById(id)
